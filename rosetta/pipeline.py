...
class MultimodalHFModel(MultimodalModelBase, MultimodalPreTrainedModel, GenerationMixin):
    ...
    def _prepare_attention_mask_for_generation(
        ...
        if not any(batch_full_attn_slices):
            return None
    ...
    def prepare_inputs_for_generation(
        ...
        if self.args.use_mot:
            ...
            if past_key_values is not None and und_abs is not None and gen_abs is not None:
                ...
                und_local = map_absolute_to_local_token_indices(
                    input_pos, und_abs, device
                )
                gen_local = map_absolute_to_local_token_indices(
                    input_pos, gen_abs, device
                )
                if kwargs["mode"] == "gen_text" and und_local.numel() == 0 and gen_local.numel() == 0:
                    und_local = torch.arange(
                        input_pos.shape[1], device=device
                    ).unsqueeze(0).expand(input_pos.shape[0], -1)
                model_inputs["und_token_indices"] = und_local
                model_inputs["gen_token_indices"] = gen_local
            else:
