#!/usr/bin/env bash
# Evaluate COCO30K (FID + CLIP Score + HPSv2, 8 GPUs, batch=32)
# Usage:  bash scripts/eval/eval_coco.sh
# Override: CKPT_DIR=<hf_weights>  SAMPLE_OUT=<output>  bash scripts/eval/eval_coco.sh
# Need 99min
...
EVAL_METRICS="coco30k@@metric=fid+clip_score+hpsv2"
TARGET="${SAMPLE_OUT}/coco30k__fid+clip_score+hpsv2/metric_results/fid.json"
