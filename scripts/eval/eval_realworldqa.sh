#!/usr/bin/env bash
# Evaluate RealWorldQA (real-world visual reasoning, 8 GPUs, instruct template)
# Usage:  bash scripts/eval/eval_realworldqa.sh
# Override: CKPT_DIR=<hf_weights>  SAMPLE_OUT=<output>  bash scripts/eval/eval_realworldqa.sh
# 4min
...
EVAL_METRICS="realworldqa@@metric=realworldqa@@max_new_tokens=10"
TARGET="${SAMPLE_OUT}/realworldqa__realworldqa_10/metric_results/realworldqa.json"
