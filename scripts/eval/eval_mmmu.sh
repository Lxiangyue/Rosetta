#!/usr/bin/env bash
# Evaluate MMMU (vision understanding, 8 GPUs, instruct template)
# Usage:  bash scripts/eval/eval_mmmu.sh
# Override: CKPT_DIR=<hf_weights>  SAMPLE_OUT=<output>  bash scripts/eval/eval_mmmu.sh
# Need 13min
...
EVAL_METRICS="mmmu@@metric=mmmu@@max_new_tokens=32"
TARGET="${SAMPLE_OUT}/mmmu__mmmu_32/metric_results/mmmu.json"
