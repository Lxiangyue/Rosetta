#!/usr/bin/env bash
# Evaluate POPE (hallucination, 8 GPUs, instruct template)
# Usage:  bash scripts/eval/eval_pope.sh
# Override: CKPT_DIR=<hf_weights>  SAMPLE_OUT=<output>  bash scripts/eval/eval_pope.sh
# Nedd 25min
...
EVAL_METRICS="pope@@metric=pope@@max_new_tokens=5"
TARGET="${SAMPLE_OUT}/pope__pope_5/metric_results/pope.json"
