#!/usr/bin/env bash
# Evaluate ARC-Challenge (logit-based, 8 GPUs, ~2 min)
# Usage:
#   bash scripts/eval/eval_arc_c.sh
# Override paths via env vars:
#   CKPT_DIR=<hf_weights_dir>  SAMPLE_OUT=<output_dir>  bash scripts/eval/eval_arc_c.sh
# Need 2min
...
EVAL_METRICS="arc_challenge@@metric=arc_challenge@@max_new_tokens=1"
TARGET="${SAMPLE_OUT}/arc_challenge__arc_challenge_1/metric_results/arc_challenge.json"

ASSETS_BASE="${ASSETS_BASE:-${REPO_DIR}/public_assets}"

if [ ! -d "${CKPT_DIR}" ]; then
    print_msg "$_NAME Checkpoint not found: ${CKPT_DIR}"
    exit 1
fi

if [ -f "${TARGET}" ]; then
    print_msg "$_NAME Already done: ${TARGET}"
    exit 0
fi
