...
# Stage 2.2: Full MMU Training (MoT Baseline)
# Continue from Stage 2.1, train projector + backbone with MMU + LM data
# Default: single-node (8 GPUs). For multi-node, set HOST_NUM=8

export ASSETS_BASE="${ASSETS_BASE:-public_assets}"
export CKPT_DIR="${CKPT_DIR:-outputs/stage2_1_projector_mot/ckpt/0003000}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/stage2_2_mmu_mot}"
...
bash launch/workers/run_train.sh torchrun \
    train/configs/stage2_2_mmu_mot.yaml \
    --ckpt-dir "${CKPT_DIR}" \
