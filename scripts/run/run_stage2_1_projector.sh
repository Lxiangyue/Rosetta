...
# Train only the projector, freeze backbone, use MMU caption data
# Default: single-node (8 GPUs). For multi-node, set HOST_NUM=8

export ASSETS_BASE="${ASSETS_BASE:-public_assets}"
export CKPT_DIR="${CKPT_DIR:-outputs/stage2_init}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/stage2_1_projector}"
...
bash launch/workers/run_train.sh torchrun \
    train/configs/stage2_1_projector.yaml \
    --ckpt-dir "${CKPT_DIR}" \
