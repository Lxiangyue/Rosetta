...
# Stage 1: Language Model Training (MoE Baseline)
# Train MoE (12 routed experts) on text-only data
# Default: single-node (8 GPUs). For multi-node, set HOST_NUM=8

export ASSETS_BASE="${ASSETS_BASE:-public_assets}"
export CKPT_DIR="${CKPT_DIR:-outputs/Qwen3-0.6B-Base-upcycling-moe-scale05}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/stage1_lm_moe}"
...
bash launch/workers/run_train.sh torchrun \
    train/configs/stage1_lm_moe.yaml \
    --ckpt-dir "${CKPT_DIR}" \
