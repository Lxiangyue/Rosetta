...
# Stage 1: Language Model Training (MoT precursor)
# Train the und-only 7-expert checkpoint before Stage 3 expands it to MoT.
# Default: single-node (8 GPUs). For multi-node, set HOST_NUM=8

export ASSETS_BASE="${ASSETS_BASE:-public_assets}"
export CKPT_DIR="${CKPT_DIR:-outputs/Qwen3-0.6B-Base-upcycling-mot-7e-scale05}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/stage1_lm_mot}"
...
bash launch/workers/run_train.sh torchrun \
    train/configs/stage1_lm_mot.yaml \
    --ckpt-dir "${CKPT_DIR}" \
